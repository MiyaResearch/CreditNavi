...
## 14b. どこでも運賃表（§ft・2026-08-08 実装）

東京メトロの各駅に掲出されている「東京メトロ きっぷ運賃表」を、全144駅ぶん任意の駅で見られる機能。

### 14b.1 画面・導線
- ホーム（検索画面）の**最下部に青いボタン**「東京メトロ どこでも運賃表」（`.ft-entry` / `openFareTable()`）
- 遷移先は push 画面 `#scr-faretable`。タイトル・説明文・出発駅の入力ボックス・路線図マップの構成
...
| ファイル | 内容 |
|---|---|
| `運賃表ベース.svg` | 路線図のベース画像（線・同一駅リンク・バッジ枠・駅名） |
| `map_tokyometro.json` | 駅座標・ラベル位置・路線パス・同一駅リンク（33KB） |

どちらも初回オープン時に遅延 fetch する（`ftEnsureLoaded()`。起動を重くしないため）。
`sw.js` の `APP_SHELL` に両方を追加し、`CACHE_NAME` を `v14` に更新済み。

座標の作り方と再生成手順は `運賃/運賃表機能_仕様設計.md` を参照。
